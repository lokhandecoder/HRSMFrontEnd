...
export const StatusTableUtilitites = () => {
  ...
  const handleReminder = async (appliedLeaveTypeId: number | undefined) => {
    setLoading(true);
    try {
      // Check if appliedLeaveTypeId is undefined, provide a default value of 0
      const idToSend =
        appliedLeaveTypeId !== undefined ? appliedLeaveTypeId : 0;

      var response = await SendLeaveReminder(idToSend);
